...
| 机制 | 说明 |
|------|------|
| 事件驱动调度 | 功能块只在事件到达时执行，节点空闲时零 CPU 占用（对比 PLC 周期扫描） |
...
| 一致性保障 | 领导者租约 + epoch fencing token，杜绝脑裂；旧纪元指令被全集群拒绝 |
...
- **事件即消息**：跨节点事件统一封装为 `Message`（含 msg_id/优先级/epoch），与 IEC 61499 中"事件输入/输出 + WITH 数据关联"一一对应；
- **资源即运行时**：每个节点进程 = 一个 IEC 61499 Device，`DistributedRuntime` 承担资源（Resource）角色，负责功能块实例化与事件连接；
- **定时器也是事件**：心跳巡检、状态同步等周期行为通过定时线程注入事件（E_CYCLE 语义），调度引擎本身不存在扫描循环；
...
| 6 | 脑裂防护 | epoch fencing，旧主控恢复后自动让位、暂停派发 |
